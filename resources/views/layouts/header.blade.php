 <!------ DESKTOP-HEADER-SECTION-START  ------>
 <div class="desktop-header">
     <header>
         <div class="navigation-wrap  start-header start-style ">
             <div class="container">
                 <div class="row">
                     <div class="col-4 col-sm-5 col-md-5">
                         <ul class="list-unstyled mb-0">
                             <li class="d-inline-block">
                                 <a class="navbar-brand" href="index.html" target="_blank">
                                     <embed src="{{asset('assets')}}/images/logo.svg" width="40px">
                                 </a>
                             </li>
                             <li class="d-inline-block">
                                 <div class="search-container">
                                     <form>
                                         <div class="outer_search">
                                             <div class="saerch-bar" id="search_bars">
                                                 <span class="material-icons">
                                                     search
                                                 </span>
                                                 <input type="text" placeholder="Search Food Items or Users"
                                                     name="search">
                                             </div>
                                             <button class="yellow-bg">SEARCH</button>
                                         </div>
                                         <div class="search-dropdown" id="scrolling">
                                             <!--<div class="search-items">
                                                 <h6 class="grey-text">FOOD ITEMS</h6>
                                                 <ul class="list-unstyled mb-3">
                                                     <li class="d-inline-block"><img class="seaarch-img"
                                                             src="{{asset('assets')}}/images/search-img1.jpg" alt="search-img1">
                                                     </li>
                                                     <li class="d-inline-block">
                                                         <h5>Tuwo Rice (25kg)</h5>
                                                     </li>
                                                 </ul>
                                                 <ul class="list-unstyled mb-3">
                                                     <li class="d-inline-block"><img class="seaarch-img"
                                                             src="{{asset('assets')}}/images/search-img2.jpg" alt="search-img1">
                                                     </li>
                                                     <li class="d-inline-block">
                                                         <h5>Tuwo Rice (50kg)</h5>
                                                     </li>
                                                 </ul>
                                                 <ul class="list-unstyled mb-3">
                                                     <li class="d-inline-block"><img class="seaarch-img"
                                                             src="{{asset('assets')}}/images/search-img1.jpg" alt="search-img1">
                                                     </li>
                                                     <li class="d-inline-block">
                                                         <h5>Tuwo Rice (25kg)</h5>
                                                     </li>
                                                 </ul>

                                             </div>
                                             <div class="search-users">
                                                 <h6 class="grey-text">Users</h6>

                                                 <ul class="list-unstyled mb-3">
                                                     <li class="d-inline-block"><img class="seaarch-list"
                                                             src="{{asset('assets')}}/images/list-img1.jpg" alt="search-img1">
                                                     </li>
                                                     <li class="d-inline-block">
                                                         <h5>Tumininu Tayo</h5>
                                                     </li>
                                                 </ul>

                                             </div>
                                             -->
                                         </div>
                                     </form>
                                 </div>
                             </li>
                         </ul>
                     </div>
                     <div class=" col-8 col-sm-7 col-md-7 ">
                         <nav class="navbar navbar-expand-md navbar-light float-right text-center ">
                             <div class="collapse navbar-collapse " id="navbarSupportedContent ">
                                 <ul class="navbar-nav drop-menu ">
                                     <li class="nav-item pl-4 pl-md-0 ml-0 ml-md-4 ">
                                         <a class="nav-link active " href="index.html ">
                                             <p class="mb-0 "><i class="material-icons-outlined ">home</i></p>
                                             <p class="mb-0 ">Home</p>
                                         </a>
                                     </li>
                                     <li class="nav-item pl-4 pl-md-0 ml-0 ml-md-4 ">
                                         <a class="nav-link " href="# ">
                                             <p class="mb-0 "><span class="material-icons ">
                                                     people_outline
                                                 </span></p>
                                             <p class="mb-0 ">Users</p>
                                         </a>
                                     </li>

                                     <li class="nav-item pl-4 pl-md-0 ml-0 ml-md-4 ">
                                         <a class="nav-link " href="# ">
                                             <p class="mb-0 "><span class="material-icons-outlined ">
                                                     shopping_cart
                                                 </span></p>
                                             <p class="mb-0 ">Cart</p>
                                         </a>
                                     </li>
                                     <li class="nav-item pl-4 pl-md-0 ml-0 ml-md-4 ">
                                         <a class="nav-link " href="# ">
                                             <p class="mb-0 "><span class="material-icons-outlined ">
                                                     shopping_bag
                                                 </span></p>
                                             <p class="mb-0 ">Order</p>
                                         </a>
                                     </li>
                                     <li class="nav-item pl-4 pl-md-0 ml-0 ml-md-4 ">
                                         <a href='#' class="nav-link ">
                                             <p class="mb-0 "><span class="material-icons ">
                                                     more_horiz
                                                 </span></p>
                                             <p class="mb-0 ">More <span class='toggle'></span></p>
                                         </a>
                                         <ul class='dropdown p-0  nav-clr'>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span class="material-icons ">
                                                         account_balance_wallet
                                                     </span> Wallet </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span class="material-icons ">
                                                         notifications</span> Notifications </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span class="material-icons ">
                                                         share
                                                     </span> Refer and Earn </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span class="material-icons ">
                                                         favorite_border
                                                     </span>Saved Products </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span class="material-icons ">
                                                         error_outline
                                                     </span>Frequently Asked Questions </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span class="material-icons ">
                                                         settings_cell
                                                     </span>Contact Admin </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span class="material-icons">
                                                         location_city
                                                     </span>Change City</a>
                                             </li>
                                         </ul>
                                     </li>

                                     <li class="nav-item pl-4 pl-md-0 ml-0 ml-md-4 ">
                                         <a href='#' class="nav-link ">
                                             <p class="mb-0 "><span class="material-icons ">
                                                     account_circle
                                                 </span></p>
                                             <p class="mb-0 ">My Account <span class='toggle'></span></p>
                                         </a>
                                         <ul class='dropdown p-0 nav-clr'>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span
                                                         class="material-icons-outlined ">
                                                         account_circle
                                                     </span> Profile </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span
                                                         class="material-icons-outlined ">
                                                         location_on
                                                     </span> Manage Address </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span
                                                         class="material-icons-outlined ">
                                                         lock
                                                     </span> Change Password </a>
                                             </li>
                                             <li class="nav-item pl-4 pl-md-0 ml-0 ">
                                                 <a href='#' class="nav-link "> <span
                                                         class="material-icons-outlined ">
                                                         login
                                                     </span>Logout </a>
                                             </li>

                                         </ul>
                                     </li>

                                 </ul>
                             </div>

                         </nav>
                     </div>
                 </div>

             </div>
         </div>
     </header>
 </div>
 <!------ DESKTOP-HEADER-SECTION-END  ------>
 <!------ MOBILE-HEADER-SECTION-START ------>
 <section class="mobile-header">
     <div class="container">
         <div class="d-flex mb-3">
             <div class=" justify-content-start">
                 <img class="mobile-logo" src="{{asset('assets')}}/images/main-logo.svg" alt="mobile-logo">
             </div>
             <div class=" ml-auto justify-content-end">
                 <img class="profile-img" src="{{asset('assets')}}/images/profile_img.jpg" alt="profile-img">
             </div>
         </div>
     </div>
     <div class="mobile-search">
         <div class="container">
             <div class="search-container" data-toggle="modal" data-target="#search-modal">
                 <form>
                     <div class="saerch-bar">
                         <div>
                             <span class="material-icons">
                                 search
                             </span>
                             <input type="text" placeholder="Search " name="search">
                         </div>
                         <!-- <button class="yellow-bg">SEARCH</button> -->
                     </div>
                 </form>
             </div>
         </div>
     </div>
 </section>
 <!------ MOBILE-HEADER-SECTION-END ------>
